...
function changeVideo(url) {
    ...
    if (videoContainer) {
        ...
        if (url.includes('youtube.com') || url.includes('youtu.be')) {
            ...
            embedCode = `<iframe src="https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false" width="560" height="315" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>`;
        } else if (url.includes('instagram.com')) {
            const instagramUrl = new URL(url);
            const postId = instagramUrl.pathname.split('/')[2];
            embedCode = `<iframe width="560" height="315" src="https://www.instagram.com/p/${postId}/embed" frameborder="0" scrolling="no" allowtransparency="true"></iframe>`;
        } else if (url.includes('tiktok.com')) {
            const tiktokUrl = new URL(url);
            const videoId = tiktokUrl.pathname.split('/').pop();
            embedCode = `<blockquote class="tiktok-embed" cite="${url}" data-video-id="${videoId}" style="max-width: 605px; min-width: 325px;">
                <section></section>
            </blockquote>
        ...
        if (embedCode) {
            ...
            alert('URL de video no válida. Por favor, introduce una URL de YouTube, Facebook, Instagram o TikTok.');
        }
    }
}
